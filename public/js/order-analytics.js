...
class OrderAnalytics {
    ...
    updateRecentOrders() {
    // Susun order dari terbaru ke terlama
    this.orders.sort((a, b) => b.createdAt - a.createdAt);
    
    // Render untuk setiap kategori dengan struktur table yang betul
    this.renderOrdersForCategory('all', this.orders);
    this.renderOrdersForCategory('shopee', this.orders.filter(o => o.platform && o.platform.toLowerCase().includes('shopee')));
    this.renderOrdersForCategory('tiktok', this.orders.filter(o => o.platform && (o.platform.toLowerCase().includes('tiktok') || o.platform.toLowerCase().includes('lazada'))));
    this.renderOrdersForCategory('qilah', this.orders.filter(o => o.team_sale && o.team_sale.toLowerCase().includes('qilah')));
    this.renderOrdersForCategory('nisya', this.orders.filter(o => o.team_sale && o.team_sale.toLowerCase().includes('nisya')));
    this.renderOrdersForCategory('wiyah', this.orders.filter(o => o.team_sale && o.team_sale.toLowerCase().includes('wiyah')));
...
renderOrdersForCategory(category, orders, page = 1) {
    ...
    if (paginatedOrders.length === 0) {
        ...
        row.innerHTML = `<td colspan="9" style="text-align: center; color: #6b7280; font-style: italic;">Tiada data order ditemui untuk kategori ini</td>`;
    ...
    paginatedOrders.forEach((order, index) => {
        const row = document.createElement('tr');
        const orderDate = order.createdAt ? new Date(order.createdAt).toLocaleDateString('ms-MY') : 
                         (order.tarikh ? new Date(order.tarikh).toLocaleDateString('ms-MY') : 'N/A');

        // Tentukan platform class untuk styling
        const platformClass = this.getPlatformClass(order.platform);
        
        // Generate product details HTML yang sepadan dengan gambar
        const productDetailsHTML = this.generateProductDetailsHTML(order);
        
        // Calculate total quantity dari products jika ada, kalau tidak default ke 1
        const totalQty = this.calculateTotalQuantity(order);
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="order-invoice">
                    <strong style="color: #60a5fa;">${order.nombor_po_invoice || 'N/A'}</strong>
                </div>
            </td>
            <td>
            ...
            <td>
                <span class="order-platform ${platformClass}">
                    ${order.platform || 'N/A'}
                </span>
            </td>
            ...
            <td style="text-align: right;">
                <span style="color: #10b981; font-weight: 700; font-size: 1rem;">
                    RM ${order.total_rm ? order.total_rm.toFixed(2) : '0.00'}
                </span>
            </td>
        `;
...
// Helper function untuk generate product details HTML seperti dalam gambar
generateProductDetailsHTML(order) {
    // Jika order ada detailed products (dari PDF parsing)
    if (order.products && Array.isArray(order.products) && order.products.length > 0) {
        const productCount = order.products.length;
        const totalSizes = this.getTotalUniqueSizes(order.products);
        
        let html = `
            <div class="product-summary">
                <span class="product-count-badge">${productCount} produk</span>
                <span style="color: #94a3b8; margin-left: 0.5rem;">${totalSizes} saiz berbeza</span>
            </div>
            <div class="product-list">
        `;
        
        // Show first 2 products, then "show more" if needed
        const displayProducts = order.products.slice(0, 2);
        
        displayProducts.forEach(product => {
            const sizeBadges = this.generateSizeBadges(product.sizes || []);
            html += `
                <div class="product-item">
                    <div class="product-name">${product.name || product.sku}</div>
                    <div class="product-qty">${product.totalQty || 0}</div>
                </div>
            ...
            html += `
                <div style="text-align: center; margin-top: 0.5rem;">
                    <span style="color: #60a5fa; font-size: 0.75rem; font-style: italic;">
                        +${order.products.length - 2} produk lagi...
                    </span>
                </div>
            `;
        }
    ...
    // Jika order standard (single product)
    else {
        return `
            <div class="product-summary">
                <span class="product-count-badge">1 produk</span>
                <span style="color: #94a3b8; margin-left: 0.5rem;">saiz campuran</span>
            </div>
            <div class="product-list">
                <div class="product-item">
                    <div class="product-name">${order.jenis_order || order.code_kain || 'N/A'}</div>
                    <div class="product-qty">1</div>
                </div>
                <div style="margin-top: 0.25rem;">
                    <span style="background-color: #64748b; color: white; padding: 0.125rem 0.375rem; border-radius: 12px; font-size: 0.7rem; font-weight: 600;">
                        Mixed
                    </span>
                </div>
            </div>
...
// Helper function untuk generate size badges
generateSizeBadges(sizes) {
    if (!sizes || sizes.length === 0) {
        return `<span style="background-color: #64748b; color: white; padding: 0.125rem 0.375rem; border-radius: 12px; font-size: 0.7rem; font-weight: 600;">Mixed</span>`;
    }
    
    return sizes.map(size => {
        const bgColor = this.getSizeColor(size.size);
        return `<span style="background-color: ${bgColor}; color: white; padding: 0.125rem 0.375rem; border-radius: 12px; font-size: 0.7rem; font-weight: 600; margin-right: 0.25rem;">${size.size}</span>`;
    }).join('');
}

// Helper function untuk dapatkan warna size
getSizeColor(size) {
    const sizeColors = {
        'S': '#ef4444',      // red
        'M': '#f59e0b',      // yellow
...
// Helper function untuk kira total unique sizes
getTotalUniqueSizes(products) {
    const allSizes = new Set();
    products.forEach(product => {
        if (product.sizes && Array.isArray(product.sizes)) {
            product.sizes.forEach(size => allSizes.add(size.size));
        }
    });
    return allSizes.size;
}

// Helper function untuk kira total quantity
calculateTotalQuantity(order) {
    if (order.products && Array.isArray(order.products)) {
        return order.products.reduce((total, product) => {
            return total + (product.totalQty || 0);
        }, 0);
    }
    
    // Fallback untuk order standard
    return order.total_quantity || 1;
}

// Helper function untuk platform class
